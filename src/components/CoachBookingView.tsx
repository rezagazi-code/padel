...
export const CoachBookingView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* Header Banner */}
      <div className="rounded-3xl bg-gradient-to-r from-slate-900 via-slate-900 to-indigo-950/70 border border-slate-800 p-6 sm:p-8 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="text-left bg-slate-950/70 border border-slate-800 p-4 rounded-2xl shrink-0">
            <span className="text-xs text-slate-400 block">جلسات آموزشی رزرو شده شما:</span>
            <span className="text-xl font-black text-[#a3e635]">{coachBookings.length} جلسه</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {coaches.map((coach) => (
          <div
            key={coach.id}
            className="rounded-3xl bg-slate-900/90 border border-slate-800 p-6 space-y-5 hover:border-slate-700 transition flex flex-col justify-between shadow-lg"
          >
            <div>
              {/* Profile Top */}
              <div className="flex gap-4">
                <img
                  src={coach.avatar}
                  alt={coach.name}
                  className="w-20 h-20 rounded-2xl object-cover ring-2 ring-[#a3e635]/50 shrink-0"
                />
                ...
                <div className="space-y-1 min-w-0">
                  ...
                  <p className="text-xs text-[#a3e635] font-bold">{coach.title}</p>
              ...
              <div className="mt-4 space-y-3 text-xs">
                <p className="text-slate-400 leading-relaxed text-[11px] bg-slate-950/60 p-3 rounded-xl border border-slate-800/80">
                  {coach.bio}
                </p>

                <div>
                  <span className="text-slate-400 text-[11px] font-bold block mb-1.5">تخصص‌ها:</span>
                ...
                <div>
                  <span className="text-slate-400 text-[11px] font-bold block mb-1.5">باشگاه‌های محل آموزش:</span>
                  <div className="flex flex-wrap gap-1.5">
                    {coach.clubs.map((c, i) => (
                      <span
                        key={i}
            ...
            <div className="pt-4 border-t border-slate-800 flex items-center justify-between gap-4">
              <div>
                <span className="text-[10px] text-slate-400 block">نرخ هر ساعت تمرین:</span>
              ...
              <button
                ...
                className="px-6 py-3 bg-[#a3e635] hover:bg-[#8fd126] text-slate-950 font-black text-xs rounded-xl shadow-md transition active:scale-95 cursor-pointer flex items-center gap-1.5"
              >
          ...
          <div className="w-full max-w-lg rounded-3xl bg-slate-900 border border-slate-800 p-6 shadow-2xl space-y-4 my-8 text-xs">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-black text-white">رزرو جلسه با {selectedCoach.name}</h3>
              </div>
              <button
            ...
            <form onSubmit={handleConfirmBooking} className="space-y-4">
              {/* Select Club */}
              <div>
                <label className="block text-slate-300 font-bold mb-1">انتخاب باشگاه تمرین:</label>
                <select
                  value={selectedClub}
              ...
              {/* Session Type */}
              <div>
                <label className="block text-slate-300 font-bold mb-1">نوع جلسه تمرینی:</label>
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`p-2.5 rounded-xl border text-right transition cursor-pointer ${
                        sessionType === typ.id
                          ? 'bg-[#a3e635]/15 border-[#a3e635] text-white'
                          : 'bg-slate-950 border-slate-700 text-slate-300'
                      }`}
                    >
                      <span className="font-bold block text-xs">{typ.label}</span>
                      <span className="text-[10px] text-slate-400 block mt-0.5">{typ.sub}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-bold mb-1">روز جلسه:</label>
                  <select
                    value={selectedDate}
                ...
                <div>
                  <label className="block text-slate-300 font-bold mb-1">ساعت جلسه:</label>
                  <select
                    value={selectedHour}
              ...
              <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
                <span className="text-slate-400 text-[10px]">مشخصات ورزشکار:</span>
                <p className="text-white font-bold">{playerProfile.name} ({playerProfile.phone})</p>
                <p className="text-[#a3e635] text-[11px]">سطح فعلی: {playerProfile.level.toFixed(2)}</p>
              </div>

              <div className="pt-2 flex gap-3">
                <button
                  type="submit"
                  className="flex-1 py-3 bg-[#a3e635] hover:bg-[#8fd126] text-slate-950 font-black rounded-xl cursor-pointer flex items-center justify-center gap-2"
                >
      ...
      {showReceiptModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm rounded-3xl bg-slate-900 border border-[#a3e635]/50 p-6 shadow-2xl space-y-4 text-xs">
            <div className="text-center space-y-2">
              <div className="w-14 h-14 rounded-2xl bg-[#a3e635]/20 border border-[#a3e635] text-[#a3e635] flex items-center justify-center mx-auto">
                <CheckCircle2 className="w-7 h-7" />
              </div>
              <h3 className="text-base font-black text-white">جلسه مربیگری با موفقیت رزرو شد</h3>
              <p className="text-slate-400 text-[11px]">پیامک هماهنگی برای مربی و شما ارسال گردید.</p>
            </div>
            ...
            <div className="rounded-2xl bg-slate-950 p-4 border border-slate-800 space-y-2">
              <div className="flex justify-between">
                ...
                <span className="text-white font-bold">{showReceiptModal.coachName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">باشگاه:</span>
                <span className="text-white font-bold">{showReceiptModal.clubName}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-[#a3e635] font-black">{showReceiptModal.price.toLocaleString('fa-IR')} تومان</span>
              </div>
            </div>

            <button
              onClick={() => setShowReceiptModal(null)}
              className="w-full py-2.5 bg-[#a3e635] hover:bg-[#8fd126] text-slate-950 font-black rounded-xl cursor-pointer"
            >
              متشکرم، بستن
            </button>
          </div>
